...
for s = 1:totalPix
    if idxTrain(s) == true
        p = p + 1;
        trainSetPix(p,:) = signals(s,:);
        trainSetMoist(p) = moisture(s);
    end

    if idxTest(s) == true
        q = q + 1;
        testSetPix(q,:) = signals(s,:);
        testSetMoist(q) = moisture(s);
    end
end
...
for div = 1:8
    ...
    [m s2] = gp(hyp, @infGaussLik, meanfunc, covfunc, likfunc, trainSetPix, trainSetMoist, testSetPix);
    
    error(smooth) = rmse(testSetMoist, m);
    end
