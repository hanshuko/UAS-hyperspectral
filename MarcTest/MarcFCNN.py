...
fcnn_model.compile(optimizer=opt, loss='mse', metrics=['mae'])

hist = fcnn_model.fit(X_train_nn, y_train, epochs=epochs, batch_size=batch_size, validation_split=0.1, verbose=1)

# Predict on training set
y_train_pred = fcnn_model.predict(X_train_nn)

# Evaluate CNN Model (testing set)
y_pred_cnn= fcnn_model.predict(X_test_nn)
