...
# Fit model
gpr.fit(X_train_pca, Y_train_final)

# Predict
Y_train_pred, Y_train_std = gpr.predict(X_train_pca, return_std=True)
Y_test_pred, Y_test_std = gpr.predict(X_test_pca, return_std=True)
...
plt.title('PCA + GPR: Predicted vs Actual Moisture on Test Set')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title("PCA + GPR: Predicted vs Actual")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()